...
## 6. 진입·로딩 화면 양식 (v1.29 · v3.3 로고 위치 고정 · v3.19 로그인 분리)

진입 계열 화면(스플래시·진입(SignupScreen)·전면 로딩)은 같은 골격 (인트로는 v3.3 삭제 —
README §제거된 기능 대장):

> **로그인 화면은 이 골격에서 빠진다 (v3.19 · 2026-08-25 사용자 지시).**
> `login_screen.dart` 에는 BrandLogo 를 넣지 않는다 — 제목 '로그인' 부터 시작하는
> 일반 폼 화면이다. 창구가 하나라 역할 선택 UI 도 없다 (판정은 서버).
