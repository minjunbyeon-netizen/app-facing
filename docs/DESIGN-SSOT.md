...
- **화면 하나에 세로로 이어지는 구역이 많으면 구역 자체를 `FkAccordion` 으로 접는다.**
...
- **작은 라벨 조각은 `FkBadge` 하나뿐이다.** 표시(읽기 전용)든 선택(토글)이든 같은 위젯을 쓴다.
...
- 모양을 바꿔야 하면 **`fkit.dart` 의 FkBadge 부터** 고친다. 한 화면만 다르게 하고 싶다는 요구는
