...
| `HkSwitch` | **켬/끔 토글 유일 규격** (v3.36) — r1 사각 트랙 44×26 + 사각 썸, 터치 48. 상태와 무관하게 같은 크기(행 높이 불변). `activeColor` 는 켜졌어도 실제로는 작동하지 않을 때(권한 차단) 중립색으로 내리는 자리 |
...
진입 계열 화면(스플래시·전면 로딩)은 같은 골격 (인트로는 v3.3, 진입 갈림길 화면은
v3.31 삭제 — README §제거된 기능 대장 11·31):

> **로그인 흐름의 화면에는 BrandLogo 를 넣지 않는다 (v3.19 · 2026-08-25 사용자 지시).**
> 로고가 남는 자리는 **스플래시와 전면 로딩(HkLoadingScreen)** 둘뿐이다.
>
> **앱 첫 화면 = 로그인 (v3.31 · 2026-08-27 사용자 승인, 브리프 D66).** 스플래시 다음이
> 곧바로 `login_screen.dart` 다 — 로그인·가입을 고르게 하던 갈림길 화면(구 `SignupScreen`)은
> 삭제했다. 제목 '로그인' 부터 시작하는 일반 폼이고, 그 아래 작은 줄(HkButton.tertiary)로
> '회원 가입 신청', 맨 밑에 이용약관 · 개인정보처리방침.
> 창구가 하나라 역할 선택 UI 도 없다 (판정은 서버).
> `HkEntryLogoGap` 은 구 진입 화면의 로고 자리 맞춤용이라 현재 소비처가 없다 —
> 로고 있는 진입 계열 화면이 다시 생길 때를 위해 HKit 에 존치.

```
┌──────────────────────────┐
│   HkEntryLogoGap (24%)   │   ← 화면 높이 24% 고정 간격 — 콘텐츠 높이와 무관하게
│     BrandLogo (220)      │      로고가 항상 같은 자리 (전환 때 로고가 안 뜀)
│                          │      ※ v3.31 현재 이 두 칸을 쓰는 화면은 없다
│   [화면별 콘텐츠 슬롯]      │   ← 스플래시: (하단 앵커) 스피너
│                          │      전면 로딩: HkLoadingScreen (로고+스피너+캡션)
└──────────────────────────┘  패딩 sp5 · 배경 bg
```

- **로고 위 간격은 Spacer·중앙정렬 금지 — `HkEntryLogoGap` 만** (v3.3 · 2026-08-21
...
- **영문 유지 (도메인 고정어만)**: 브랜드 `HYPHEN` · 도메인 용어 `AMRAP` `EMOM` `RX`
  `RX+` `Scaled` `Elite` `Games` `1RM` `PR` `UB` `Metcon` `For Time` `Engine` `Split` `Burst` ·
  단위 `kg/lb` · `XP` · 동작명(Thruster 등) · 명언(원문).
- **크로스핏 표기 철수 (v3.0 · 2026-08-14 사용자 지시 "이 제품은 크로스핏장이 아니다")**:
  노출 문구에서 `WOD`·박스·크로스핏 금지 — 박스→**체육관**, WOD 게시물→**수업 내용**,
  시간표 면→**수업 시간**, 크로스핏 경력→**운동 경력**. 고유명사(Fran·Girls/Hero/Games WOD·
  CrossFit Open/Games·명언 원문)와 내부 코드·API·DB 이름은 예외 (GLOSSARY §2 v2·§4).
- **문장 톤**: 명사형 간결체 ("오늘의 수업" · "불러오기 실패"). 완곡·응원·이모지 금지는 유지.
  버튼 = 동사 명령형 2~4자 ("저장" "다음" "다시 시도"). "~하세요" 남발 금지, 안내문은 "~합니다/됩니다" 격식.
- **혼용 허용**: 도메인 영단어 + 한글 조사 결합 허용 ("1RM 을 갱신했습니다") — 구 V9 금지 폐기.
- **금지 용어**: 박스·크로스핏·WOD(일반 명칭)·헬스·다이어트·웰니스·체중관리·쉬운·편리한·누구나·
  당신·귀하 (copy_lint). '운동' 은 v3.0 에서 해제 ('운동 경력' 정본화).
...
- 목록이 길면 상단 헤더에 `n / m` + "전체 보기", 본문은 **최대 5줄** + "그 외 N개" 마지막 행.
  - **예외 — 홈 업적 섹션은 3줄**(`achievement_section.dart kRows`). 로딩·빈·데이터 세 상태의
    높이를 같게 예약하려면 상한이 곧 예약 높이가 되는데, 5줄을 예약하면 해금이 적은 회원에게
    빈 자리가 과하게 남는다 (2026-08-27 D68 · §레이아웃 안정성).
...
> 현재 적용: **8 파일 20 검사** — 로그인·가입·온보딩·홈·내 정보·업적·코치 주간·쪽지함·
> 쪽지 상세·수업 상세·수업 탭·주간 보드·요일 띠·여닫기·완료 시트
> (`test/golden/*stability*_test.dart` · 헬퍼 `layout_stability.dart`).
> 새 화면에 적용할 때는 상태 목록 + 앵커 키만 주면 된다.
> 화면별 커버리지·밀림 후보 = `docs/UI-INDEX.md §3·§4`.

### 로딩·빈·에러는 같은 바닥을 갖는다 (D115 · 2026-09-04)

> 사용자 지시 "스켈레톤으로 자리 잘 비워두고 특히 레이아웃 시프트 발생 안 하도록."

셋을 같은 자리에서 갈아 끼우는데 실측 높이가 **로딩 22 · 빈 70(캡션 97) · 에러 131** 로
달라 최대 **109px** 밀렸다. 규격이 부품에 없어 호출부가 감싸 준 곳만 안전했다.

- `HkEmptyState` · `HkErrorState` · **`HkLoading.slot()`** 셋이
  `HyphenTokens.stateSlotH`(132)를 **최소 높이**로 갖는다. 내용이 크면 그 위로 자란다.
- **`HkLoading()` 기본 생성자는 22 그대로다** — 버튼 안·줄 안처럼 이미 자리가 있는 곳용.
  빈·에러와 같은 자리에서 갈아 끼울 때는 반드시 `.slot()` 을 쓴다.
- 게이트 = `test/state_slot_test.dart` — 상수를 비교하지 않고 **실물을 렌더해
  `tester.getSize` 로 잰다** (`touch_target_test.dart` 와 같은 방식).
- PC 관리자 웹의 같은 규칙 = `web/facing-admin/design/SSOT.md §17` (용어는 여기가 정본,
  그쪽은 웹에서의 적용만 적는다).
>
> **빈 띠 없이 자리를 지키는 법 (2026-08-28 '알림 받기')**: 표 행처럼 부제 슬롯이
> 이미 있는 자리는 **부제를 상태와 무관하게 항상 한 줄 두고 글자만 바꾼다**.
> 안내가 붙었다 빠지는 것이 아니라 문장이 교체되므로 높이가 처음부터 끝까지 같고,
> 아무 일 없을 때도 남는 빈 띠가 없다 — D69 의 실패를 피하면서 밀림도 막는다.
> 고정 높이 슬롯(HkNoticeSlot)은 부제를 둘 자리가 없는 폼·목록에 쓴다.
>
