...
진입 계열 화면(스플래시·진입(SignupScreen)·전면 로딩)은 같은 골격 (인트로는 v3.3 삭제 —
README §제거된 기능 대장):

> **로그인 흐름의 화면에는 BrandLogo 를 넣지 않는다 (v3.19 · 2026-08-25 사용자 지시).**
> 진입(`signup_screen.dart`)·로그인(`login_screen.dart`) 둘 다 로고 없이 간다.
> 로고가 남는 자리는 **스플래시와 전면 로딩(HkLoadingScreen)** 둘뿐이다.
> 진입 화면은 `HkEntryLogoGap` 만 유지해 버튼 스택을 스플래시 로고와 같은 높이에
> 고정하고, 로그인 화면은 제목 '로그인' 부터 시작하는 일반 폼이다.
...
│                          │      ※ 진입 화면은 이 칸을 비운다 (v3.19)
│   [화면별 콘텐츠 슬롯]      │   ← 진입: 버튼 스택 / 스플래시: (하단 앵커) 스피너
