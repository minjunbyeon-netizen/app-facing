...
v1.22 개편 이후 패치 세션 2회차. seed_personas 실행 + 벤치마크 여성 M/F 탭 구현 + 에뮬레이터 실동작 검증 완료.
...
- [x] **seed_personas 실행** — `services/facing/data/seed_personas.py`
  - profiles 10 / memberships 6 / history 80 / today_wod 2 생성 완료
  - Notice FAB 코치에게 정상 표시 가능 상태
- [x] **벤치마크 여성 M/F 탭** — `lib/core/benchmark_data.dart` + `lib/features/home/benchmark_sheet.dart`
  - `BenchmarkMetric`에 `femaleTierValues` / `femaleNote` + `valuesFor(female)` / `noteFor(female)` 메서드
  - 6카테고리 전부 F-constants 반영 (grading.py SSOT):
    POWER·OLYMPIC·GYMNASTICS·CARDIO·METCON·BODY (SMM% 신규 추가)
  - `_BenchmarkSheet` → StatefulWidget, 우상단 M|F TextButton 토글
  - `GestureDetector` → `TextButton` (DraggableScrollableSheet 드래그 충돌 해결)
  - 에뮬레이터 `emulator-5554` 실동작 확인 ✅
...
- [ ] **갤럭시 실기 QA** — 에뮬레이터 그리드 탭으로만 확인, 손가락 M/F 토글 실기 검증 권장
...
| 역할 | 경로 |
|---|---|
| 벤치마크 데이터 (M+F) | `lib/core/benchmark_data.dart` |
| 벤치마크 시트 UI (M/F 토글) | `lib/features/home/benchmark_sheet.dart` |
