...
- **갤S22 실기 검증 (15:55 · 프로드 읽기 전용)** — 폰 IP 가 바뀌어 있었다
  (구 `192.168.1.103` → `172.30.1.41` → **`172.30.1.50:5555`** — 같은 날에도 두 번 바뀌었다.
  `tools/phone/last-addr.txt` 갱신. 안 붙으면 `172.30.1.x` 대역을 5555 로 훑는다).
...
### 4. 저녁 세션 추가분 (D74~D79 · 빌드 3014 → 3017)

- **D74** 칭호(Panel B) 일체 제거 — 채울 수 없는 값을 읽어 영영 안 풀리던 기능. 업적으로 통일.
- **D75** PC 홈 날짜 화살표 (`?date=`) · **D76** 수업 일정 시간표 순 아코디언 + 예약자 이름 → 회원 상세 링크
  (서버 `reserved_members` `[{id,name}]`) · PC 토스트 2배.
- **D77** 앱 수업 탭 칸 순서 **프로그램 · 수업 시간**, 프로그램이 기본 진입 · 갈 때 오늘 펼침.
- **D78** 프로그램 칸 **첫 수업 시각 순 · 같은 종류 한 번만 · 전부 펼침**, 카드 제목 `AWAKE · FOR TIME`
  (서버 `first_class_at`·`template_name` 동봉).
- **D79** 수업 안내 회원 노출(체육관 정보 카드 '수업 종류', 이벤트 배지) + **공지 실시간 갱신**
  (`GymState.announcementsChanged` → `AnnouncementsState.refresh`; 서버는 수정·삭제도 SSE).
  공지가 안 보이던 진짜 원인 = 앱이 첫 응답을 굳히고 SSE 로 다시 묻지 않던 것. 서버 필터는 정상이었다.
- 오늘 **세 번 반복된 유형**: 서버·데이터는 있는데 **화면 경로가 끊김** (칭호 벤치마크 · 결제축 매출 ·
  수업 안내). 다음 점검 갈래로 삼을 것 — "서버가 주는데 아무도 안 읽는 키" 스캔이 값싼 신호였다.
- **D80** 공지 한 줄 전광판 `HkMarquee` (넘칠 때만 흐름 · 세 자리 공용 · 접근성 '애니메이션 줄이기' 존중).
  프로드 공지 `eeeee` 는 짧아 서 있는 게 맞고, 로컬 긴 공지로 에뮬 2프레임 비교해 흐름 실증.
- 테스터 링크 = `github.com/minjunbyeon-netizen/app-hyphen/releases` (최신 **3018**).
- 실기 SSE 검증은 **로컬 서버**로 회원 기기 구독 상태에서 posted/updated/deleted 3종 도착 확인.
  갤S22 실화면 캡처는 폰이 PIN 잠금(Bouncer)이라 못 했다 — APK 는 설치돼 있다.
...
- [x] ~~'칭호' 화면(Panel B)이 채울 수 없는 값을 읽는다~~ → **2026-08-29 17:00 제거 완료**
      (사용자 "그럼 없애야지, 지금 업적으로 하는거잖아"). 브리프 **D74** · README 대장 37.
      게이미피케이션은 **업적 하나**로 통일. 아래는 발견 당시 기록.
- [ ] ~~(발견 기록)~~
...
## 20:16 프로드 500 사고 — 원인·교훈 (같은 날 20:32 핫픽스)

- **증상**: 폰 "서버 오류가 발생했습니다" — 수업 탭이 열릴 때마다 `GET /gyms/2/wods` 500.
- **원인**: D78 에서 `gym.py list_wods` 에 넣은 `first_class_at` 분기가 `kst_wall` 을 쓰는데
  그 파일이 import 하지 않았다 (`NameError`). **내 실수.**
- **왜 검사가 못 잡았나**: 그 분기는 `template_id` 가 있는 글에서만 도는데 기존 검사 데이터엔
  그게 없었다 → 503 건 전부 초록인 채 프로드만 죽었다. 앱 골든도 가짜 API 라 못 잡는다.
- **조치**: `kst_date`(이미 import 된 것)로 교체 · **그 분기를 반드시 타는 회귀 검사** 추가
  (옛 코드에서 정확히 그 NameError 로 실패하는 것 확인) · 서버 전체 `pyflakes` undefined name 0 ·
  배포 후 프로드 500 재발 0 · 갤S22 실화면 정상.
- **교훈 (다음 세션 강제)**: 서버에 **새 분기**를 넣으면 그 분기를 타는 데이터로 검사를 같이 넣는다.
  "전부 초록" 은 그 분기를 한 번도 안 돌렸다는 뜻일 수 있다. 값싼 사전 게이트 = 배포 전 `pyflakes`.
