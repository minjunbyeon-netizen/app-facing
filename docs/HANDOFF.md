...
- **D81** 홈 공지 검정 전광판(`HkMarquee(fill: true)` — 짧은 글도 채워 항상 흐름) · 공지는 홈에서만 ·
  수업 탭 하단 체육관 정보·공지 삭제 · 내 정보 주소·체육관 기록·내 체육관 삭제 (README 대장 38).
  ⚠ **결정 대기**: 수업 탭 체육관 정보가 사라지며 D79 '수업 종류(수업 안내)' 노출 자리도 같이 사라졌다.
  `GymInfoCard` 호출처 0곳(위젯·모델·저장소·골든 보존). 어디에 둘지(홈 카드 / 내 정보 메뉴 / 폐기) 사용자 결정.
- 테스터 링크 = `github.com/minjunbyeon-netizen/app-hyphen/releases` (최신 **3019**).
