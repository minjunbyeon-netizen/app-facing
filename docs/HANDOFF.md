...
- [ ] **HYPEE 온보딩 등장 연출 — 안 선택 대기** (2026-08-21 19:30 별도 세션).
      전신 액션 11종이 "샥샥샥" 나타나는 연출 4안을 웹 데모로 만들어 검증까지 끝냈다.
      데모 = `C:\dev\services\design\_작업\앱에셋\온보딩연출데모.html`
      **인계 정본(이식 절차·좌표·함정) = `C:\dev\services\video\docs\HANDOFF.md`**
      대표가 ① 1~4안 중 하나 ② 스플래시에 얹기(A) vs 인트로 부활(B) 를 정하면 착수.
      ⚠ B 는 오늘 v3.3(`fb7ae8c`)에서 지운 `/intro` 를 되살리는 것이라 반드시 확인받을 것
