...
- [ ] **갤S22 실기 검증** — 이번엔 에뮬레이터까지만 했다.
- [ ] **미수금 환불 차단만 프로드 미검증** — 결제 행을 만들어야 해서 안 했다
      (로컬 실서버 왕복·pytest 로는 통과).
- [ ] **매출 정가 축 합칠지** — `admin.gym_stats`(회원권 정가 × 시작일) 2곳이
      이원화 baseline 3 으로 남아 있다. 결제 축과 **다른 사실**이라 합칠지는 사용자 결정.
      (결제 축 두 창구는 D70 에서 `_metrics.payment_revenue_in_month` 로 합쳤다.)
...
`/resume` → 갤S22 실기 검증(오늘 흐름 재확인) 또는 매출 정가 축 합칠지 결정.
