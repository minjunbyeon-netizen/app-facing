...
> 상태: **승인 (2026-08-20 사용자) → P1·P2 + P3 일부 구현 완료 (같은 날)** —
> 주 시작 = 월요일(ISO) · 미소급(streak 연속만 인정) · custom 인증 기본 = 코치 승인.
> **+카테고리 3분류 지시**: 설정·빌더를 행동의 관측 방식으로 나눈다 —
> ①자동(출석·예약) ②기록 로그(수업 기록·PR) ③코치 인증(커스텀).
> 카테고리는 trigger 에서 파생 (attendance→1 · wod_log/pr→2 · custom→3) — 별도 컬럼 없음.
>
> **구현 로그 (2026-08-20)**: P1 백엔드(3표·엔진·훅 3곳·API·회귀 11건 — 전체 153) ·
> P2 PC 빌더(카테고리 3섹션·문장형 모달·인증 대기함, playwright 실기동) ·
> P3 중 해금 축하(토스트+컨페티 캐논 2초, diff 감지)와 완료 기록 시트 개편
> (수업 내용 인계·동작별 SCALED/RXD·ELITE 제거) — 에뮬레이터에서 규칙 생성→
> 기록 저장→50P 적립+업적 해금+컨페티까지 3차 실검증.
> **P3 도전 카드는 2026-08-20 밤 구현·실검증 완료** — 홈 도전 섹션(문장+진행바+
> 달성 ✓+승인 대기)+[인증하기] 시트(1일 1회), 에뮬레이터에서 인증 접수→PC 승인→
> 진행률 1/2 반영까지 실기 왕복. 골든 45장 (member_21).
> **P4 트리거 4종(예약·결제·연장·생일)은 2026-08-20 저녁 구현 완료** — 상세
> 명세·확정 규칙 = `PLAN-record-structures.md` Part B (§8 표의 P4 행 갱신).
...
| **P3 앱 표시** | 도전 카드 + [인증하기] 시트 + icon 렌더 + 업적 상세 포인트 줄 / 골든 갱신 — **2026-08-20 완료** | ✅ |
| **P4 확장** | 트리거 4종(예약·결제·연장·생일) — **2026-08-20 구현 완료** ("좋네" 승인, 명세 = PLAN-record-structures.md Part B) / point_rule 통합 검토·알림(해금 푸시)은 잔여 | 트리거 ✅ |
