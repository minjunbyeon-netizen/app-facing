...
> **2026-09-03 갱신 (심사 제출 직전 점검)**: 버전 **`1.0.0+3025`** 로 AAB 재빌드(49.9MB, 운영 URL 주입) ·
...
10. **애플 동시 출시 목표 (2026-08-28 사용자 지시) — iOS 준비는 `docs/APPSTORE.md` 가 정본.** 같은 날
    ...
    이후 기능 커밋으로 **현재 `1.0.0+3025`** (2026-09-03 AAB 재빌드). 3008·3009 산출물은 폐기. 이 문서의 옛 번호 표기는 이력.
...
8. **앱 등록 후 AAB 업로드.** `build/app/outputs/bundle/release/app-release.aab` (**1.0.0+3025 · 49.9MB**, 2026-09-03 재빌드, 릴리즈 키 서명·운영 URL 주입 완료) 그대로 사용 가능.
...
- [x] 릴리즈 AAB 빌드 성공 (`build/app/outputs/bundle/release/app-release.aab`, **1.0.0+3025 · 49.9MB — 2026-09-03 재빌드**)
...
5. ● **테스트 → 클로즈드 테스트 → 트랙 만들기** → 테스터 이메일 목록 등록(12명 이상 여유) → **AAB 업로드**
   (`build/app/outputs/bundle/release/app-release.aab`, **1.0.0+3025** — 3008·3009 는 폐기) → 출시 노트 한 줄 → 검토 후 출시.
