...
업적
출석과 수업 기록이 쌓이면 업적이 열립니다. 첫 열 번, 3주 연속, 첫 PR — 달성하는 순간 화면이 축하하고, 모은 업적은 내 정보에 칭호로 남습니다.
...
5. ● **테스트 → 클로즈드 테스트 → 트랙 만들기** → 테스터 이메일 목록 등록(12명 이상 여유) → **AAB 업로드**
   (`build/app/outputs/bundle/release/app-release.aab`, 1.0.0+3009 — 3008 은 폐기) → 출시 노트 한 줄 → 검토 후 출시.
