...
> **2026-09-03 갱신 (심사 제출 직전 점검)**: 버전 **`1.0.0+3026`** 로 AAB 재빌드(49.9MB, 운영 URL 주입) ·
> 스크린샷 7장·아이콘·피처 그래픽을 09-02 골든(4탭 셸) 기준으로 재생성(옛 세대 파일 정리) · 심사 계정 개명
> (**`testcoach1`·`testmember1`** — 옛 googletest1·2 삭제, A-12 표) · 공개 방침·삭제 페이지에서 사라진 '착용 칭호' 항목 삭제 ·
> **건강 앱(Health apps) 선언** 항목 추가(F-6) · 요건 재확인: 신규 앱 target API 36(2026-08-31~) 충족 · 개인 계정 테스터 12명·14일 유지 ·
> 16KB 페이지 크기 NDK 28 기본(하드 차단은 2027-02-01) · 방침 URL 전 앱 필수 · 계정 삭제 앱 내+웹 둘 다 필수(둘 다 있음).

---

## 0. 최우선 블로커 요약

1. ~~Play Console 개발자 계정이 아직 없다.~~ **등록 신청 완료 (2026-08-28, 사용자) — 신원 인증 대기 중.** 인증이 끝나면 **§G 런시트** 순서로 당일 진행.
...
4. ~~심사용 회원 계정 값이 아직 확정 전이다.~~ **해소됨 (2026-08-28).** 코치 `googletest2`·회원
   ...
   **2026-09-03 개명 (사용자 "이름 안 헷갈리게")**: 코치 `testcoach1`(표시 이름 테스트코치1) · 회원 `testmember1`(테스트회원1).
   옛 `googletest1`·`googletest2` 는 운영에서 삭제. 비밀번호 두 개 다 Railway 변수 (A-12 표).
...
9. **스토어 양식에 넣는 URL 4개 — 전부 확정 (2026-08-28).** 구글 플레이·앱스토어 공통.
   ...
   커스텀 도메인은 없다 — Railway 기본 도메인이 곧 공식 주소. 지원 이메일 = `cheb2oy@naver.com` (삭제 요청 페이지·홈페이지 푸터와 동일 —
...
10. **애플 동시 출시 목표 (2026-08-28 사용자 지시) — iOS 준비는 `docs/APPSTORE.md` 가 정본.** 같은 날
    ...
    이후 기능 커밋으로 **현재 `1.0.0+3026`** (2026-09-03 AAB 재빌드). 3008·3009 산출물은 폐기. 이 문서의 옛 번호 표기는 이력.
...
8. **앱 등록 후 AAB 업로드.** `build/app/outputs/bundle/release/app-release.aab` (**1.0.0+3026 · 49.9MB**, 2026-09-03 재빌드, 릴리즈 키 서명·운영 URL 주입 완료) 그대로 사용 가능.
...
10. **스토어 등록정보 에셋 업로드.** 전부 제작 완료 — `build/store/` 폴더 그대로 올리면 된다.
    ...
    - 폰 스크린샷: `phone_01`~`phone_07` (각 1080×1920). 순서 = 다섯 가지 본연 (2026-08-28): 수업 예약 → 공지 → 쪽지(꽉 찬 쪽지함) → 업적 달성 순간 → 업적 목록 → 홈 레벨·포인트 → 내 정보 포인트.
...
12. **앱 액세스 안내에 심사팀용 테스트 로그인 계정 입력.** 로그인 없이는 아무 화면도 못 보는 앱이라 이 항목을 비우면 반려된다.
    ...
    **심사용 계정 2개 (2026-08-28 생성 → 2026-09-03 개명·재생성, 운영 서버 로그인 200 확인 — 구 googletest1/2 는 삭제).**

    | 역할 | 아이디 | 비밀번호 | 만든 방법 | 확인 |
    |---|---|---|---|---|
    | 코치 | `testcoach1` (표시 이름 테스트코치1) | Railway 환경변수 `REVIEW_COACH_PASSWORD` | 부팅 시 시드 (`models/base.py`, 이름 = `REVIEW_COACH_NAME`, 옛 아이디 정리 = `REVIEW_RETIRE_COACH_IDS`) | 로그인 200, 역할 coach, 체육관 HYPHEN(gym 2) |
    | 회원 | `testmember1` (표시 이름 테스트회원1) | Railway 환경변수 `REVIEW_MEMBER_PASSWORD` (서버가 읽지 않는 메모 변수) | 가입 신청 API + 코치 승인 + 회원권 '심사용 회원권'(~2027-12-31, 0원) | 로그인 200, 상태 approved, 체육관 HYPHEN, 에뮬 실로그인 확인 |
    | 회원 (계정 삭제 검증 전용) | `testmember2` (표시 이름 테스트회원2) | Railway 환경변수 `REVIEW_MEMBER2_PASSWORD` | 같은 절차 (2026-09-03) | 로그인 200, approved. 심사팀이 '계정 삭제' 를 눌러도 되는 계정 — 지침에 명시 |
    ...
    > - 회원 비밀번호는 Railway `REVIEW_MEMBER_PASSWORD` 에 메모해 두었다 (2026-09-03 — 서버는 읽지 않는다).
    >   분실하면 같은 방식(가입 신청 + 승인 + 회원권)으로 새 계정을 하나 더 만들면 된다.
    ...
    - 코치용 데모 계정 `admin` / `1234` 도 운영에서 살아 있고 로그인된다(200, role boss). 다만
      **심사팀에는 `testcoach1` 을 주는 것을 권한다** — `admin` 을 속도 제한 예외에 넣으면
    ...
    - 콘솔 입력 시 지침 문구 예: "아이디·비밀번호로 로그인하십시오. 코치 계정은 수업·회원 관리
      화면, 회원 계정은 홈·수업·히스토리·내 정보 4개 탭을 볼 수 있습니다. 두 계정 모두 같은 체육관(HYPHEN)에 속합니다."
      (영문 병기 권장 — 구글 지침이 안내 문구를 영어로도 요구: "Sign in with ID and password. The coach account shows
      the reservation roster and inbox; the member account shows Home, Classes, History and My Info tabs. Both belong to the same gym.")
...
> **입력 정본 = `docs/STORE-SUBMIT-SHEET.md`** (2026-09-03 — 콘솔 화면 순서·복사용). 이 절과 §D 는 근거·이력이며,
> 값이 시트와 어긋나면 시트를 따르고 이쪽을 고친다. 제출 직전 게이트 = `python tool/store_preflight.py`.

카피 규칙 적용: 금지 용어(박스·크로스핏·WOD·헬스·다이어트·건강·체중관리·웰니스·칼로리 소모·"쉬운"·"편리한"·"누구나"·당신·귀하) 미사용, 이모지 없음, 명사형 간결체. 제품 실체 = 수업 예약·공지·쪽지·업적·포인트 다섯 가지 (2026-08-28 사용자 지시 '본연에 집중' — 홈페이지·스크린샷과 같은 순서).
...
```
체육관 코치와 회원을 잇는 앱. 수업 예약·공지·쪽지, 그리고 업적과 포인트로 운동을 재밌게.
```
...
- Railway `service-facing` 환경변수에 `REVIEW_LOGIN_IDS=googletest1,googletest2` 등록 완료 → **2026-09-03 `testcoach1,testmember1` 로 교체** (계정 개명, 옛 아이디 삭제).
...
- [ ] Play Console 신원 인증 대기 (등록·결제는 2026-08-28 완료) — 인증 후 §G 런시트
...
- [x] 릴리즈 AAB 빌드 성공 (`build/app/outputs/bundle/release/app-release.aab`, **1.0.0+3026 · 49.9MB — 2026-09-03 재빌드**)
...
- [x] 이용약관 URL 공개 (`/terms`)
- [x] 폰 스크린샷 7장 (`build/store/phone_01`~`07`, 각 1080×1920 = 1:1.78 — 2:1 상한 안) — **2026-09-03 재생성** (09-02 골든·4탭 셸 반영, 옛 세대 파일 삭제)
...
- [x] 심사용 계정 2개 운영 서버 생성·로그인 확인 — 코치 `googletest2`(시드), 회원 `googletest1`(가입 신청 + 코치 승인). 둘 다 200 (A-12)
  — **2026-09-03 개명·재생성**: 코치 `testcoach1`(테스트코치1, 시드) · 회원 `testmember1`(테스트회원1, 가입+승인+회원권). 옛 googletest1·2 와 잔여 테스트 회원 행('차감문구 검증') 삭제. 운영 로그인 200 둘 다 + 에뮬 실로그인 확인
...
- [ ] **건강 앱(Health apps) 선언** — 앱 콘텐츠 탭에 뜨는 양식. 이 앱은 수업 완료 기록(동작별 무게·한 횟수)을 저장하므로
  "활동 및 피트니스(Activity and fitness) — 운동 기록" 에 해당으로 답한다. 건강 데이터(심박·체중 등)·Health Connect 는 없음
  (2026-09-03 조사 — support.google.com/googleplay/android-developer/answer/14738291)
- [ ] (개인 계정 선택 시만) 클로즈드 테스트 12명·14일 진행
- [ ] 프로덕션 심사 제출
- [ ] **제출 직전 `python tool/store_preflight.py` 전부 PASS** (URL·심사 계정 로그인·에셋 규격·AAB·권한·iOS 설정 실측 — 2026-09-03 신설)

---

## G. 인증 완료 당일 런시트 (2026-08-28 사용자: 개발자 등록 2곳 신청 완료 · 인증 대기)

> 인증 메일이 오면 이 순서대로. Claude 가 할 수 있는 것은 ★, 사용자만 할 수 있는 것은 ●.

1. ● 콘솔 로그인 → 개발자 정보란(이름 변민준·이메일·전화·주소) 입력 (A-7). 사업자·통신판매업 칸은 비운다.
2. ● **앱 만들기** — 이름 HYPHEN · 기본 언어 한국어 · 앱/게임 = 앱 · 무료.
3. ● **앱 콘텐츠**(정책) 탭 — 개인정보처리방침 URL · 앱 액세스(심사 계정 2개, A-12) · 광고 없음 · 콘텐츠 등급 설문(D-2) ·
   타겟층(D-3) · 뉴스 앱 아님 · 데이터 보안(D-1) · 정부 앱 아님 · 금융 기능 없음(F-6). 전부 이 문서 답안 그대로.
4. ● **스토어 등록정보** — 이름·간단 설명·자세한 설명(§C) · 아이콘 `icon_512.png` · 피처 `feature_1024x500.png` ·
   스크린샷 `phone_01~07` · 카테고리(D-4) · 연락처 이메일 `cheb2oy@naver.com` · 웹사이트(§0-9).
5. ● **테스트 → 클로즈드 테스트 → 트랙 만들기** → 테스터 이메일 목록 등록(12명 이상 여유) → **AAB 업로드**
   (`build/app/outputs/bundle/release/app-release.aab`, **1.0.0+3026** — 3008·3009 는 폐기) → 출시 노트 한 줄 → 검토 후 출시.
   첫 업로드 때 **Play 앱 서명(Play App Signing) 동의**를 반드시 켠다 — 서명 키가 이 PC 한 대에만 있기 때문.
6. ● 테스터에게 옵트인 링크 전달 (아래 초대 문구) → 12명이 14일 연속 설치 유지.
7. ★ 그동안 Claude: 사전 출시 보고서(Robo) 결과 확인 · 크래시/ANR 대응 · 필요 시 3010 재빌드.
8. ● 14일 경과 → **프로덕션 액세스 신청** → 승인 후 프로덕션 트랙에 같은 AAB(또는 최신) 출시 → 심사.

### 테스터 초대 문구 (카카오톡·문자용)
> HYPHEN 앱 테스트에 참여해 주세요. 아래 링크에서 "테스터 되기"를 누른 뒤 Google Play 에서 설치하고,
> 2주 동안 지우지 말고 두어 주시면 됩니다. 로그인은 체육관에서 쓰는 아이디 그대로입니다.
> 링크: (클로즈드 테스트 트랙의 옵트인 URL — 콘솔 트랙 페이지 "테스터" 탭에서 복사)
