...
> **D118 (2026-09-05 집행) — 밀림 잔여 2건: 홈 도전 섹션 · 계약 목록/상세 (앱).
> D117 이 "정직하게 남긴다" 고 적어 둔 4 중 둘을 마저 집행했다.**
>
> - **순서를 또 지켰다.** 게이트를 먼저 얹어 **6 실패 / 3 통과**를 확인하고, 고쳐서
>   **9 전부 통과**. 잰 값: 홈 도전 섹션 로딩·0건·실패 **0px** ↔ 도전 2건 **289px**
>   (홈 스크롤 총길이 145 → 434) · 계약 목록·상세 첫 줄 **68 ↔ 405 = 337px**
>   (본문 자리 높이 **728**, 기대 132).
...
> - 안정성 검사 **22 → 31** (`grep -c testWidgets` · 10 파일 기준. D117 의 '25' 는
>   세는 기준이 달랐다 — 같은 기준으로 다시 세어 적는다). 전체 **265 통과** ·
>   analyze 0 · 골든 **89장 유지, 1장 재생성**(`member_13_achievement_detail` —
>   홈이 길어져 시트 **뒤 배경**의 `scrollUntilVisible` 도달 위치가 달라졌다.
>   시트 자체는 그대로).
> - **남은 밀림 후보 2**: 수업 상세 대체요청 시트(앵커 밖) · 채팅 전송 아이콘
>   (**미확인** — 실측이 먼저). 목록 = `docs/UI-INDEX.md §12`.
