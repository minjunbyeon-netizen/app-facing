...
> **D82 (2026-08-29 사용자 지시) — 예약 오픈 전 수업 줄에 플레이스홀더 문구 (앱).**
>
> 사용자 원문: "회원폰에서 수업볼때 예약설정에서(지정한 시간이 아니여서 예약버튼을 활용할수없음에도)
> 즉, 안되더라도, 플레이스 홀더로 보여줘야함 — 예를들어 '예약가능한 시간이 아닙니다' 이런 느낌이 있어야함."
>
> - 종전(D58)엔 예약 오픈 전 수업의 우측 슬롯이 작은 회색 배지 **'오픈 전'** 이었다 — 왜 못 누르는지가
>   줄에서 안 읽혔다(탭해야 스낵바로 시각이 나왔다). 이제 그 자리에 문장 **'예약 가능한 시간이 아닙니다'**
>   가 선다 (`HkSlotNote` 신설 — HKit §3 코드·클래스 SSOT, 배지와 같은 자리·caption 두 줄 안이라 줄 높이 불변).
> - 탭은 종전대로 서버로 보내 409 `BOOKING_NOT_OPEN` 문구("예약은 8/30 11:00 부터 가능합니다.")를
>   스낵바로 — 정확한 오픈 시각의 정본은 서버 하나(앱에 시각 계산을 두 번 적지 않는다).
> - 문구 정본 = `lib/features/classes/class_line.dart kBookingNotOpenNote` — 골든 `state_15` 검사가 같은 상수를 본다.
>   개념어 '오픈 전'(GLOSSARY)은 그대로 — 표시 문장만 바뀐다. '회원권 필요'·'마감'·'종료' 배지는 그대로.
> - 회귀: `flutter test` 전량 · `analyze` 0 · 골든 71장(state_15 재생성, 수 변화 없음). 빌드 **3020**.
...
>   새 행 기본 11). 목록 응답 `booking_open_at` 동봉 → 폰 `ClassSessionDto.isBookingNotOpen` → 주간보드 우측 슬롯
>   문구 **'예약 가능한 시간이 아닙니다'**(D82 · 2026-08-29 — 구 '오픈 전' 배지 교체. 탭하면 서버 문구 스낵바 —
>   정책 문구는 서버 하나). 보기는 종전대로 언제나.
