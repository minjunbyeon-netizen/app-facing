# §7 User Journey — 첫 사용 30초 가치 도달 · 3 모드 분기 · 페르소나별 분기 · 5 메인 탭 · Retention 흐름

> **문서 버전**: v1.1 · 2026-04-28
> **대상 독자**: PM / Eng Lead / UX Designer
> **소속 섹션**: ABOUT.md §7 확장 (6-pager 형식)
> **단일 진원지**: 이 문서가 User Journey 흐름의 SSOT. ABOUT.md §7 요약은 이 문서를 참조.
>
> **v1.1 변경점**: Tier 부여 후 3 모드 분기 (Coach / Member / Solo) 추가. §3 T6 / §4-6, §4-7 / §6 모드 분포 / §9 FAQ Q11~13 / Appendix mode 컬럼 신설.
...
> **Splash → Tier 부여까지 30초. Tier 직후 5초 안에 역할(Coach/Member/Solo) 분기. 이후 모든 것은 Tier 를 강화하기 위한 구조다.**
...
Tier 직후 ModeSelect (3 카드: Coach / Member / Solo) 1 화면을 거치고 Main Shell 진입. 모드는 영구 결정 아님 — Settings 에서 언제든 변경. 분기 단계가 추가됐지만 페이싱 가치(Tier) 는 ModeSelect 이전에 이미 도달.
...
Tier 직후 Coach / Member / Solo 1개를 선택한다. 모드는 SharedPreferences `app_mode` 키에 저장된 클라이언트 분기 도구. 사용자 정체성이 아니라 **현재 사용 의도**를 표현한다.

| Mode | 의미 | 가입 조건 |
|---|---|---|
| Coach | 박스 운영자. 와드 게시·멤버 승인·Coach Dashboard 접근. | CreateGym 진입(스킵 허용 — 추후 등록) |
| Member | 박스 가입자. 코치 와드·노트·숙제 수신. | FindGym 진입(스킵 허용 — Calc 만 사용) |
| Solo | 박스 미사용. Calc + Trends + Attend + Profile. | 추가 입력 없음 |

**원칙**:
- 모드 = 클라이언트 UI 게이트. 백엔드 권한은 여전히 `role` + `gym_status` 가 결정.
- 한 사람이 다중 정체성 보유 가능 (예: 박스 코치이면서 다른 박스 멤버). 모드 전환으로 화면만 바뀌고 데이터는 보존.
- Settings 에서 언제든 모드 변경. 변경 시 SharedPreferences 만 갱신, 백엔드 호출 없음.

**페르소나 자동 매핑** (debug PersonaSwitcher):
| 페르소나 role | 자동 매핑 모드 |
|---|---|
| `coach_owner`, `admin` | Coach |
| `member` (approved/pending/rejected 무관) | Member |
| `app_user` | Solo |
...
CTA: `Next` → Mode Select (§4-6).

> v1.0까지 CTA = `Start WOD` → 직접 Shell 진입. v1.1 부터 ModeSelect 분기 추가로 CTA 텍스트와 다음 화면이 변경.

### 4-6. Mode Select — 역할 분기 (v1.1 신규)

**진입 조건**: `hasGrade == true` 이면서 `app_mode == null` (SharedPreferences). Splash 부트스트랩 단계에서도 같은 조건으로 직접 진입 가능.

**화면 구성**:

| 카드 | 라벨 | 한글 부제 | 설명 (· bullet 3줄) |
|---|---|---|---|
| 1 | `COACH` | 박스 운영 | Box 등록 + WOD 게시 / 멤버 관리 + 인박스 / Coach Dashboard |
| 2 | `MEMBER` | 박스 멤버 | Box 검색 + 가입 신청 / 코치 WOD 받기 / Coach 노트 + 숙제 |
| 3 | `SOLO` | 혼자 트레이닝 | Engine 측정 + 페이싱 계산 / 업적 + 시즌 배지 / 박스 기능 잠금 |

- 헤드라인 (AppBar title): `YOUR ROLE`
- 본문 헤드라인: "역할 선택" (h2)
- 본문 캡션: "코치는 박스를 운영합니다. 멤버는 박스에 가입합니다. Solo 는 혼자 트레이닝."
- 하단 안내: "언제든 Settings 에서 변경." (caption, muted)

**진입 시 자동 동작**:
1. 카드 탭 → `Haptic.medium()`
2. `AppModeStore.set(mode)` (SharedPreferences `app_mode` 저장)
3. 다음 화면으로 `pushNamedAndRemoveUntil` (뒤로가기로 Mode Select 재진입 차단)

**Settings 진입 시**:
- `arguments == 'settings'` 일 때 `automaticallyImplyLeading: true` (뒤로가기 허용)
- 모드 변경 후 `pop()` (재분기 화면 거치지 않음 — 단순히 SharedPreferences 만 갱신)

### 4-7. 모드별 분기 화면 (v1.1 신규)

> ⚠ **이 절은 폐기됐다 (v2.6 · 2026-08-13 사용자 지시 — 1인 샵 전용).**
> 박스 개설(`/onboarding/create-gym`)은 2026-06-10 에, 박스 찾기(`/onboarding/find-gym`)는
> 2026-08-13 에 라우트째 끊었다 (`lib/main.dart` 주석 처리 — 딥링크로도 안 열린다).
> 회원이 박스에 들어오는 길은 **코치가 PC 에서 발급한 가입 코드** 하나뿐이다.
> 아래 내용은 화면 파일이 보존돼 있어(“숨김 = 코드 보존”) 되살릴 때를 위한 기록이다.

ModeSelect 카드 탭 직후 진입하는 1개 화면.

**Coach → CreateGymScreen** (`/onboarding/create-gym`)

| 항목 | 유형 | 비고 |
|---|---|---|
| Box Name | 텍스트 | 필수 (스킵 시 미생성) |
| Location | 텍스트 | 옵셔널 |
| Skip 버튼 | 숨김 액션 | "추후 등록" 의도 |

- 헤드라인: `Register your box.` / 캡션: "박스 정보는 Settings 에서 추가/수정."
- 등록 시 `GymState.createGym(name, location)` 호출 → 백엔드 `POST /api/v1/gyms` → 응답 `gym_id` SharedPreferences 저장.
- Skip 또는 등록 완료 후 → `/shell` (5 탭 Shell) 진입.

**Member → GymSearchScreen** (`/onboarding/find-gym`)

| 섹션 | 내용 |
|---|---|
| 검색창 | 박스 이름·지역 검색 (자동완성 추후) |
| 결과 리스트 | Box 카드 (이름·위치·코치 수·멤버 수) |
| 가입 신청 | 카드 탭 → 신청 다이얼로그 → `POST /api/v1/gyms/:id/join` |

- 헤드라인: `Find your box.` / 캡션: "가입 신청 후 코치 승인 대기."
- 가입 신청 완료 → `/shell` 진입. WOD 탭은 pending 상태로 표시.
- Skip 또는 빈 결과 → `/shell` 진입. WOD 탭은 app_user 상태(가입 CTA만).

**Solo → 직접 Shell**

분기 화면 없음. ModeSelect 에서 SOLO 카드 탭 → `/shell` (5 탭 Shell) 진입.

---

## 5. Main — 5 탭 구조

> §4-5 (Tier) → §4-6 (Mode Select) → §4-7 (모드별 분기 화면) → 본 §5 Main Shell. ModeSelect 우회 경로 없음. Settings 에서 모드 변경 시에는 다시 §4-6 만 거치고 분기 화면 우회 (§4-6 Settings 동작 참조).
...
| 지표 | 정의 | 예상 비율 (가설) |
|---|---|---|
| Coach 비율 | ModeSelect Coach 선택 / 전체 신규 | 5~10% |
| Member 비율 | ModeSelect Member 선택 / 전체 신규 | 60~70% |
| Solo 비율 | ModeSelect Solo 선택 / 전체 신규 | 25~30% |

**해석 가이드**:
- Coach > 15%: 박스 owner 가 다중 계정으로 코치 모드 평가 중. 정상.
- Member > 80%: 박스 검색·가입 마찰 검토 필요 (FindGym 빈 결과 비율 같이 점검).
- Solo > 50%: 박스 lock-in 약화 신호. WOD 탭 가입 CTA 노출 강화 검토.
- Coach 선택 후 CreateGym 스킵 비율 > 60%: "추후 등록" 의도 우세 → 박스 등록 마찰 점검.

**모드 변경 빈도** (Settings 경유):
| 지표 | 정의 | 정상 범위 |
|---|---|---|
| 모드 변경률 | 7일간 ≥ 1회 mode 변경한 사용자 / DAU | < 5% |
| Solo→Member 전환율 | Solo 사용자 중 박스 가입한 비율 (월 단위) | ≥ 10% (lock-in 신호) |
...
| Q11. 모드는 변경 가능한가? | Settings → "MODE" 행 → ModeSelect 재진입 (`arguments: 'settings'`). SharedPreferences 만 갱신, 백엔드 호출 없음. 변경 즉시 5 탭 UI 재구성. |
| Q12. Solo 사용자가 박스에 가입하면? | (1) Settings → MODE → Member 선택 → FindGym → 가입 신청. (2) Profile 탭 박스 검색 진입점 → FindGym → 가입 신청. 두 경로 동등. 가입 후에도 Calc 탭은 동일 사용. |
| Q13. 코치가 다른 박스 멤버이기도 하면? | 백엔드 `role` + `gym_status` 다중 조합 지원. 단, 클라이언트는 1개 모드만 표현 — Coach 모드면 본인 박스 owner 화면, Member 모드면 다른 박스 멤버 화면. Settings 에서 빠르게 토글. v2 에서 단일 화면 다중 박스 표시 검토. |

---

## Appendix — 페르소나 × 탭 접근 매트릭스

| 역할 / 상태 | 자동 매핑 mode | Calc | WOD | Trends | Attend | Profile | Inbox |
|---|---|---|---|---|---|---|---|
| admin | Coach | 전체 | 전체 + 등록 | 전체 | 전체 | 전체 | 전체 |
| coach_owner | Coach | 전체 | 전체 + 등록 | 전체 | 전체 | 전체 | 4탭 |
| approved | Member | 전체 | 전체 | 전체 | 전체 | 전체 | 3탭 |
| pending | Member | 전체 | 대기 안내 | 전체 | 전체 | 전체 | 차단 |
| rejected | Member | 전체 | 재신청 CTA | 전체 | 전체 | 전체 | 차단 |
| app_user | Solo | 전체 | 가입 CTA | 전체 | 전체 | 전체 | 차단 |

Inbox 3탭 = ALL / NOTES / ASSIGNMENTS. 4탭 = +OUTBOX (coach only).

**자동 매핑 규칙** (PersonaSwitcher debug only):
- `coach_owner`, `admin` → `AppMode.coach`
- `member` (gym_status 무관) → `AppMode.member`
- `app_user` → `AppMode.solo`

매핑은 PersonaSwitcher 진입 시 1회 처리 (`AppModeStore.set` 호출). 사용자가 Settings 에서 변경 시 백엔드 role 과 무관하게 SharedPreferences 만 갱신 — 즉, app_user 가 Coach 모드를 선택해도 5 탭 UI 는 Coach 화면을 보여준다(권한 없는 와드 게시는 백엔드에서 거절).
