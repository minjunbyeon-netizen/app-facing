...
>
> **v1.1 변경점**: ModeSelect (Coach/Member/Solo) 도입에 따라 §4-1 역할 매트릭스 mode 컬럼 추가. `GymWodPost.timeCapSec` 필드 코드 정합 (Phase 1 게시 시점 입력 가능 — FAQ Q9 갱신). `GymWodComment` 모델 풀스펙 §4-5 보강. `GymWodResult.scaleLevel` 필드 §4-6 리더보드 자동 분리 명시.
...
### 4-1. 역할 정의 (v1.1 mode 컬럼 추가)

| 역할 | 식별 조건 | 자동 매핑 mode | WOD Broadcasting 권한 |
|---|---|---|---|
| Coach Owner | `role == coach_owner` AND `box.owner_id == device_id` | Coach | 게시 · 복제 · 삭제 · 리더보드 관리 |
| Approved Member | `gym_status == approved` AND 동일 박스 | Member | 수신 · 결과 입력 · 리더보드 조회 |
| Pending Member | `gym_status == pending` | Member | 차단 — WOD 탭 잠금 + 대기 안내 |
| Rejected Member | `gym_status == rejected` | Member | 차단 — 재가입 안내만 표시 |
| App User (no-gym) | 박스 미가입 | Solo | 차단 — 박스 가입 funnel 노출 |
| Admin (debug) | `role == admin` | Coach | 전체 (감사·QA 용도) |

**Mode 게이트 보강**:
- 클라이언트 `app_mode` 가 `Coach` 인데 백엔드 role 이 `app_user` (예: Solo 사용자가 모드 임의 변경) → `POST /api/v1/gyms/:id/wods` API 가 거절. UI 만 Coach 화면이 보일 뿐 실제 게시 불가.
- ModeSelect 진입 시 `Coach` 카드 → CreateGym 화면. 박스 미등록 상태에서는 게시 권한 잠금 (Coach Dashboard "박스 등록 필요" 안내).
...
5. 라운드별 세부 (round-by-round) 입력 (옵션)
   ...
   - 코드 모델: `WodRoundItem(label, content, timeCapSec)` — 라운드별 timeCap 도 가능
6. **Time Cap (옵션)** — `GymWodPost.timeCapSec` 필드에 초 단위 저장 (v1.1 코드 정합)
   - 입력 시점: 게시 화면에서 분·초 슬라이더
   - 표시: 멤버 화면 상단 배지 (예: `CAP 12:00`)
   - 자동 중단: Session Timer 자동 중단은 Phase 2 (현재는 표시만)
7. Publish → 박스 Approved Member 전체 즉시 전달
   - `GymState.postWod(postDate, wodType, content, scaledVersion, beginnerVersion, scaleGuide, roundsData, rounds, timeCapSec)` 단일 진원지
```
...
|---|---|---|
| `id` | 댓글 ID | 서버 발급 |
| `authorPrefix` | 작성자 device_hash 앞자리 | 익명 표시 (개인 정보 미노출) |
| `isMine` | 본인 댓글 여부 | UI 분기 |
| `body` | 댓글 본문 | 600자 제한 |
| `createdAt` | 작성 시각 ISO8601 UTC | UI 표시 직전 KST 변환 |

피드백 태그 / 답글 스레드 / 이미지 첨부는 Phase 2 백로그.

### 4-6. 박스 리더보드 (v1.1 모델 명시)
...
- PR 감지: 같은 WOD 명칭의 이전 기록 대비 시간 단축 → 자동 PR 알림 + Haptic (`PrDetector.countPrs` / `isPrAgainst`).
...
|---|---|---|
| `id` | 결과 ID | — |
| `rank` | 박스 내 순위 | 정렬된 순위 (서버 계산) |
| `deviceHashPrefix` | 멤버 익명 ID | UI 표시명 |
| `isMine` | 본인 결과 여부 | 강조 행 |
| `timeSec` | 총시간 (For Time) | 빠른 순 정렬 |
| `rounds` / `extraReps` | 라운드·잔여 횟수 (AMRAP/EMOM) | 많은 순 정렬 |
| **`scaleLevel`** | RX / Scaled / Beginner | **버전별 탭 자동 분리** |
| `notes` | 멤버 자유 메모 | 옵셔널 |
| `createdAt` | 입력 시각 | 동률 시 빠른 입력 우선 |

`scaleLevel` 단일 필드로 3 버전 탭 분기가 자동 처리된다 (코치가 별도 그룹 지정 불필요).
...
**Q9. For Time WOD 에서 시간 초과(time cap) 를 설정할 수 있나? (v1.1 갱신)**

가능하다. `GymWodPost.timeCapSec` 필드가 게시 시점에 초 단위 저장된다 (`GymState.postWod(...timeCapSec: 720)` 같은 형태). 멤버 화면 상단 배지로 `CAP 12:00` 표시. 라운드별 timeCap 은 `WodRoundItem.timeCapSec` 으로 라운드 단위 별도 설정 가능.

**Phase 2 에 남은 부분**: Session Timer 의 자동 중단 / time cap 도래 알림 / 강제 결과 저장. 현재는 표시만 처리되고 멤버 자율 종료에 의존.
