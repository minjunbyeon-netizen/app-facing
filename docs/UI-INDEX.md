...
## 12. 앱 밀림 잔여 집행 — 홈 도전 · 계약 (D118 · 2026-09-05)

> §11 의 **#7 · #8** 을 집행했다. 여기서도 **검사부터 썼다** — 고치기 전에 게이트를
> 얹어 실패를 먼저 확인하고(6 실패 / 3 통과), 고쳐서 9 전부 통과시켰다.

| # | 자리 | 잰 밀림 (전 → 후) | 어떻게 고쳤나 |
|---|---|---|---|
| 7 | 홈 도전 섹션 | 로딩·0건·실패 **0px** ↔ 도전 2건 **289px** · 홈 스크롤 총길이 145 → 434 | 섹션(라벨+카드)을 **항상** 세우고 카드 안만 교체 — 로딩·0건·실패가 같은 바닥(`stateSlotH` 132 ≈ 도전 한 행 128.5). 실패는 이제 '다시 시도' 로 보인다 (전엔 0건과 구분 불가) |
| 8 | 계약 목록·상세 | 첫 줄 y **68 ↔ 405 = 337px** · 본문 자리 높이 **728**(기대 132) | 본문 전체 교체를 그만두고 `ListView` + `HkReservedSlot(stateSlotH)` — 뼈대는 고정, 자리 **안**만 교체. 로딩은 카드와 같은 높이(78)의 스켈레톤이라 첫 줄이 제자리에서 내용으로 바뀐다 |

### 결정 — 0건일 때 빈 카드를 세우는 이유
...
- 새 게이트 9 = `test/golden/stability_home_challenge_test.dart`(3) ·
  `test/golden/stability_contracts_test.dart`(6). 안정성 검사 **22 → 31**
  (`grep -c testWidgets`, 10 파일).
- 골든 **89장 유지**, `member_13_achievement_detail` **1장 재생성** — 그 캡처는
  홈을 `scrollUntilVisible` 로 굴려 업적 행을 띄우는데, 홈이 도전 섹션만큼
  길어져 시트 **뒤 배경**의 스크롤 위치가 달라졌다 (시트 자체는 그대로).
...
- 2026-09-05 — **D118 밀림 잔여 2건**(§12). §11 의 #7 홈 도전 · #8 계약 목록·상세.
  §4 밀림 후보 10 중 8 해소, 2 남음(#4 시트 에러 · #10 채팅 전송 아이콘 미확인).
  앱 안정성 검사 31 · 골든 89장(1장 재생성).
