...
SECTIONS = [
    ...
    ("스낵바", "스낵바 3종 — 성격별 캐릭터 (HkSnack 단일 창구)", [
        ...
        ("snack_04_reserved", "예약 완료 — 굵은 제목 + 안내 두 줄(5분 전 도착·5분 지각 불가) (D86, 2026-08-29)"),
        ("snack_06_saving", "저장 중 — '수업을 저장 중이에요' + 가로 로딩바, 캐릭터 없음 (2026-08-30 사용자 원문)"),
        ("snack_07_saved_fighting", "저장 완료 — 하이피 '예____ 화이팅!!!!' + 저장됨·출석 +1 + 서버 비교 문구(PR) (2026-08-30 사용자 원문)"),
        ("snack_05_late_cancel_toast", "늦은 취소 — 서버 문장 'N월, N회째 레이트 캔슬입니다 … 코치에게 전송됩니다' + 차감 문구, 우는 캐릭터 (D100, 2026-08-30)"),
    ]),
    ...
    ("회원 셸", "회원 셸 3탭 — 홈 · 수업 · 내 정보 (승인된 회원 · HYPHEN CrossFit 서면)", [
        ...
        ("member_01_shell_wod", "수업 탭 (기본, 셸) — 요일 띠 + 오늘 수업 줄, 다음 수업이 펼쳐진 채 (D111 통합 한 줄, 2026-09-04)"),
    ...
    ("회원 심화", "회원 심화 — 셸에서 한 단계 더 (2026-08-19 전 화면 확장)", [
        ("member_07_classes",
         "수업 탭 (D111 통합 한 줄, 2026-09-04) — 요일 띠 + 오늘 수업 줄. 다음 수업 20:00 SWEAT 가 펼쳐져 파트 셋·메모·완료/메시지/자세히, 21:00 은 접힘(예약·대기 배지), 오늘 수업이 없는 AWAKE·BUILD 글은 아래 '프로그램' 밑"),
        ("member_26_program",
         "수업 탭 — 요일 띠에서 내일(목)을 고른 상태: AWAKE 06:00 첫 수업 자동 펼침 + '아직 게시 전.' (D111, 2026-09-04 · 구 '프로그램 칸' 캡처 대체)"),
        ("member_08_classes_reserved", "수업 예약 — 예약 확정 상태 (예약됨 + 취소 진입, 예약 줄이 펼쳐져 파트가 보인다 — D111)"),
    ...
    ("상태 변형", "상태 변형 — 빈 · 에러 · 오프라인 · 미가입", [
        ...
        ("hist_02_list", "히스토리 — 기록 목록, 검색어 없음 = 최근순. 제목 = 수업 이름 · 둘째 줄 그날 운동 요약 · 셋째 줄 종류·일시, 오른쪽 점수 라벨(+PR) (D84 → D91)"),
        ("hist_03_search", "히스토리 — 'squat' 검색: 연관도순, Back Squat(최근) · Front Squat 두 건만 (D84)"),
        ("state_28_result_sheet_movements", "완료 시트 — 동작별 기록: 한 횟수·무게 kg 칸이 코치가 정한 값으로 미리 채워짐, SCALED/RXD 칩 (D94, 2026-08-30)"),
        ("state_29_result_sheet_saving", "완료 시트 — '저장' 직후: 버튼은 자리 그대로 busy, 아래 '수업을 저장 중이에요' 로딩바 토스트 (2026-08-30 사용자 원문, 밀림 0)"),
        ("state_30_late_cancel_done", "늦은 취소 성사 순간 — 다이얼로그 확정 뒤 서버 토스트(N월 N회째 · 코치에게 전송) + 차감 문구, 폭죽 없음 (D100, 2026-08-30)"),
        ("hist_04_detail", "히스토리 상세 — 점수 라벨·난도·PR·메모·수업 내용(서버가 그린 글 그대로) + 맨 아래 '동작별 기록 보기' 배지 (D91 → 2026-09-02 동작 필터 진입점)"),
        ("hist_05_movement_filter", "히스토리 — 동작 필터: 상세의 Thruster 배지 탭 → 목록이 그 동작 기록만 (서버 ?movement_id=, 검색 칸 자리에 같은 규격 '동작: Thruster' 읽기 전용 칸 — y 불변, 2026-09-02)"),
        ("hist_06_detail_no_score", "히스토리 상세 — 점수 없는 v3.45 기록: 점수·난도 히어로 줄을 숨기고 동작별 기록만 (완료 입력에서 점수·난도 제거의 짝, E2E 실검증 발견 — 종전 '-' 64sp 검은 막대 픽스)"),
        ("hist_07_detail_capped", "히스토리 상세 — 캡 기록: 완주와 다른 단위라 '캡' 배지로 가르고, 파트별 줄을 서버 line 그대로 세운다 (D122 §2·§7, 2026-09-06)"),
        ...
        ("state_27_reservation_done", "예약 성사 순간 — 세 줄 토스트 + 화면 중앙 폭죽(350ms 프레임, 고정 시드) (D86, 2026-08-29)"),
        ...
        ("state_31_inbox_activity_error",
         "쪽지함 '활동' 칸 조회 실패 — 빈 문구로 위장하지 않고 에러+다시 시도 (2026-09-02 잔여 검증)"),
        ...
        ("state_34_result_sheet_emom",
         "완료 시트 EMOM — 맨몸 동작만 든 파트도 사라지지 않는다: 축은 '완료한 분' 한 칸"
         "(힌트 = 서버 score_target '10분 중'), 입력 칸 없는 동작은 서버 줄로 (D122 §4·§5, 2026-09-06)"),
    ]),
]
