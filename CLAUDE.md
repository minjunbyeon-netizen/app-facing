...
| 수업 탭 2칸 | **프로그램** · **수업 시간** (v3.37 분리 · **v3.40 순서 뒤집음** — 2026-08-29 사용자 "수업시간-프로그램 순서 바꾸자. 프로그램 누르면 그날 되는 운동목록이 한번에 보이게"). HkSegment 한 줄 — **기본 진입 = 프로그램**(탭을 열면 "오늘 뭐 하지"가 먼저 답해진다). 프로그램 칸으로 갈 때는 **오늘을 펼친다** (그 주에 오늘이 있을 때만). 주는 두 칸 공용. 빈 문구는 칸 이름을 따른다 — 수업 시간 '등록된 수업 없음.' / 프로그램 '아직 게시 전.'·'게시된 프로그램 없음.' **프로그램 칸(v3.41)**: 그날 프로그램을 **첫 수업 시각 순**으로 세우고(서버 `first_class_at`) 같은 수업 종류는 **한 번만**, 카드는 **전부 펼친다**. 카드 제목은 `AWAKE · FOR TIME` (수업 종류 · 운동 종류). 정본 = `week_board.dart visibleProgram`, 회귀 = `test/golden/program_order_test.dart`. **파트(D109 · 2026-09-04 사용자 "60분 운동에서 A세션때 15분 B세션때 20분 … 다른 운동이 아님")**: 같은 수업의 A·B·C 는 **한 카드 안의 파트** — 파트가 둘 이상이면 머리에 종류·캡·라운드를 안 적고 파트마다 서버 `title`(`A 파트 · 15분 · STRENGTH`) + `lines`(동작 줄) 세로, 끝에 `memo`. 파트 하나면 종전 본문. 이름표는 서버 `display_name`(수업 종류 이름), 수업 줄은 서버 `display_title`. 구 D89 "세션마다 카드 따로" 는 폐기. 앱은 '파트' 글자를 붙이지 않는다 (정본 `WodRow` · `GymWodPost.isMultiPart`) |
...
현재 **89장** (2026-09-04 — D109 파트: 사용자 "60분 운동에서 A세션때 15분 B세션때 20분 … 다른 운동이 아님".
