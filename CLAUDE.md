...
| 수업 탭 | **주 이동 줄 + 요일 띠 + 그날 수업 줄** (D111 통합 한 줄 · 2026-09-04 사용자 1안). **줄은 닫힌 채로 열린다 (D112 · 같은 날 사용자 "화살표를 눌러야 운동이 보이게, 다시 누르면 닫히게")** — 접힌 줄 = `20:00 · SWEAT ∨ · 정원 8/12 · [예약]` 넷뿐. **누를 게 없으면 배지 자리는 빈다**(D119 · 2026-09-05 — 지난 수업의 '종료', 하루·주 한도의 '오늘 예약 완료' 폐기. 높이는 48 유지). 이름 옆 화살표(또는 줄 본문)를 누르면 그 줄 아래 그날 운동이 파트 세로로 펼쳐지고(머리 없음 — 이름은 줄이 이미 말했다) 다시 누르면 닫힌다. 여러 줄 동시 열기 가능, **날짜·주를 옮기면 전부 닫힘**. 자동으로 열리는 줄 없음(구 D111 autoExpanded 폐기). 수업 줄이 없는 종류의 글은 아래 '프로그램' 라벨 밑 카드로(이것도 닫힌 채). 정본 = `week_board.dart`·`ClassLine.onToggle`, 회귀 = `test/golden/class_tab_test.dart` |
...
라 줄이 들쭉날쭉해지지 않는다. 게이트 = `test/class_row_clean_test.dart`(문구 부재 + 세 줄 높이 동일).
**2장 재생성, 장수 불변** — state_07_class_ended · state_32_class_daily_limit.
직전 **89장** (2026-09-05 — D118 밀림 후보 **잔여 4건 + 버튼 폭**: 채팅 전송 아이콘 · 수업 상세
...
둬서 누르면 운동이 보이게 하고, 다시 누르면 닫히게. 아무것도 안 누르고 날짜만 클릭해서 그 날로 돌아왔을 때는
지정한 수업시간·운동프로그램·정원·예약 혹은 종료 이런 버튼만". 들어오면 **전부 닫힘** — 접힌 줄은 시각·이름(∨)·
정원·배지 넷뿐이고 D111 요약 줄은 삭제, 자동 펼침(autoExpanded)은 폐기, '프로그램' 카드도 닫은 채. 19장 재생성,
장수 불변: member_07(수업 탭 기본 = 줄만)·member_01·member_26(내일 줄을 열어 '아직 게시 전')·member_08·
member_06b·member_09·member_10(카드·줄을 열고 진입)·state_01·07·08·11·15·22·23·27·30·32·33. 회귀 =
`test/golden/class_tab_test.dart`, 진입 헬퍼 = `harness.dart openClassRow`·`openProgramCard`. 직전 **89장** (2026-09-04 저녁 — D111 수업 탭 통합 한 줄: 사용자 진단 "수업은 수업대로 보고, 예약은 예약대로 …
