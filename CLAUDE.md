...
| 수업 탭 = 통합 한 줄 | **요일 띠 + 수업 줄(시간 · 내용 · 예약 · 완료 한 자리)** (D111 · 2026-09-04 사용자 진단 "수업은 수업대로 보고, 예약은 예약대로 … 가독성·레이아웃 통일성 떨어져서 반응 유도 어려울 것 같은데" → 목업 세 안 중 **1안** 선택). 구 두 칸(프로그램 · 수업 시간 세그먼트, v3.37~v3.41)은 **폐기** — 동기(프로그램)와 버튼(예약)을 갈라 반응까지 칸 전환이 한 번 더 들었고, 요일 아코디언 + 카드 이중 펼침에 '수업 없음' 요일 줄이 오늘 위에 쌓였다. 구조: 주 이동 줄(‹ 8.10 – 8.16 [이번 주] ›) → **요일 띠** 7칸(`HkDayStrip` — 오늘 = 채움, 고른 날 = 테두리, 점 = 내 예약 주색 / 수업 있음 회색 / 없음; 정의 `week_board.dart dayMark`) → 고른 날의 **수업 줄**(`ClassLine.member` 규격 그대로 — 시각 · `display_title` · 정원/대기 · 배지 예약/대기/예약됨/종료/오늘 예약 완료, 접힌 줄 밑에 서버 `summary` 한 줄). 줄 본문 탭 = 펼침 → 그 종류(`templateId`)의 그날 글을 **`WodRow` 본문만**(headerless — 파트 세로 · 메모 · [완료 표시/예약 필요/기록] [메시지] [자세히]), 글 없으면 '아직 게시 전.'(미래)/'게시된 프로그램 없음.', 잠김은 `LockedWodBanner`. **자동 펼침** = 내 예약 줄 전부, 없으면 다음 수업 1개, 지난 날 0개 (`autoExpanded`). 수업 종류 없는 단발 글·그날 수업이 없는 종류의 글은 목록 아래 **'프로그램'** 라벨 밑에 종전 카드(머리 포함, 전부 펼침 — `leftoverPrograms`). 수업도 글도 없으면 '등록된 수업 없음.'(HkSectionSlot 예약 자리). 순서·중복 정본 = `visibleProgram`, 회귀 = `test/golden/class_tab_test.dart` · `program_order_test.dart` · 밀림 = `stability_wod_test.dart`. **파트(D109)**: 같은 수업의 A·B·C 는 한 글 안의 파트 — 파트 둘 이상이면 파트마다 서버 `title`(`A 파트 · 15분 · STRENGTH`) + `lines` 세로, 끝에 `memo`; 파트 하나면 종전 본문. 앱은 '파트'·요약 글자를 붙이지 않는다 (정본 `WodRow` · `GymWodPost.isMultiPart`). ⚠ 수업 줄에 글이 붙으려면 서버 회원 수업 목록에 `template_id` 가 있어야 한다 — 없으면 글은 '프로그램' 밑으로 간다(사라지지 않는다) |
...
현재 **89장** (2026-09-04 저녁 — D111 수업 탭 통합 한 줄: 사용자 진단 "수업은 수업대로 보고, 예약은 예약대로 …
가독성·레이아웃 통일성 떨어져서 반응 유도 어려울 것 같은데" → 목업 세 안 중 **1안**. 두 칸(프로그램 · 수업 시간)
세그먼트·요일 아코디언 폐기 → 주 이동 줄 + **요일 띠**(`HkDayStrip`, 오늘 채움 · 고른 날 테두리 · 점 = 내 예약/수업 있음) +
고른 날의 **수업 줄**(접힌 줄 밑 서버 `summary`, 펼치면 그 종류의 글 본문만 — 파트 세로·메모·완료/메시지/자세히). 자동
펼침 = 내 예약 줄 전부, 없으면 다음 수업 1개. 수업이 없는 종류의 글은 아래 '프로그램' 라벨 밑 종전 카드. 장수 불변 —
19장 재생성: member_07_classes(오늘: 20:00 SWEAT 펼침 + 파트 셋, 21:00 접힘, 아래 AWAKE·BUILD) · member_26_program
(재정의: 요일 띠에서 내일 선택 — AWAKE 06:00 자동 펼침 + '아직 게시 전.'; 구 '프로그램 칸' 캡처 대체) · member_01_shell_wod
(셸 수업 탭) · member_08(예약 줄 펼침) · member_06·06b·09·10(시트·상세 뒤 배경 / 상세는 AWAKE 카드의 '자세히') ·
state_01·07·08·11·15·22·23·27·30·32·33(수업 탭 상태 — 같은 데이터, 새 구조). 회귀 = `test/golden/class_tab_test.dart`
(순서·자동 펼침 3규칙·leftover·summary·점) · `program_order_test.dart` · 밀림 = `stability_wod_test.dart`(요일 전환·줄
펼침 시 주 이동 줄·요일 띠 y 불변). 구 `week_pane_test.dart`·`tapProgramPane/tapSchedulePane` 삭제 (README 대장 43).
직전 89장 = 2026-09-04 낮 — D109 파트: 사용자 "60분 운동에서 A세션때 15분 B세션때 20분 … 다른 운동이 아님".
같은 수업의 A·B·C 는 카드 셋이 아니라 **한 카드 안의 파트** — member_26_program 이 SWEAT 한 장 안에
'A 파트 · 15분 · STRENGTH' / 'B 파트 · 20분 · AMRAP · 캡 12분' / 'C 파트 · 10분' 머리줄(서버 title)·동작
줄(서버 lines)·메모를 세로로 담고, 머리에는 종류를 안 적는다. 구 D89 세션(AWAKE A·B 두 카드 ·
수업 줄 'SWEAT · A 세션')은 폐기 — 20장 재생성, 장수 불변: member_26·member_01·state_33(프로그램
칸) · member_06·member_10(SWEAT 글의 시트) · member_07·08·state_07·08·11·15·22·23·27·30·32(수업
줄 제목 'SWEAT') · hist_02·04·05·06(히스토리 제목 'SWEAT'). 회귀 = `test/golden/program_order_test.dart`
파트 그룹. 직전 89장 = 2026-09-03 오후 — coach_03_shell_messages 재생성: 코치 셸(coach_shell.dart)이
