...
const List<_FaqItem> _kFaqs = [
  _FaqItem(
    '박스에 가입하려면 어떻게 하나요?',
    '코치에게 가입 코드를 받아 앱에 입력하면 바로 연결됩니다. 코드가 없으면 '
        '앱에서 가입 신청을 넣고 코치 승인을 기다리면 됩니다. 연결되면 수업 예약·출석·계약이 열립니다.',
  ),
  _FaqItem(
    '가입 신청했는데 아직 "대기" 상태예요.',
    '승인은 박스 코치가 직접 처리합니다. 오래 걸리면 박스에 직접 문의하는 게 '
        '가장 빠릅니다. 거절된 경우에도 같은 박스에 다시 신청할 수 있습니다.',
  ),
  _FaqItem(
    '수업 예약은 어디서 하나요?',
    'WOD 탭에서 날짜를 고르면 그날 수업이 함께 뜹니다. 예약을 누르면 자리가 '
  ...
  _FaqItem(
    ...
    '출석·결제 등 박스가 정한 기준에 따라 적립됩니다. 적립 기준과 사용처는 '
        '박스마다 다르니 코치에게 확인하십시오. 잔액은 프로필의 포인트 행에 표시됩니다.',
  ),
  ...
  _FaqItem(
    '내 레벨은 어떻게 정해지나요?',
    '크로스핏 경력 하나로 정해집니다. 1년 미만은 SCALED, 1~3년은 RXD, '
  ...
  _FaqItem(
    '탈퇴하면 데이터는 어떻게 되나요?',
    '프로필 → 개인정보처리방침 → 계정 삭제로 탈퇴하면 서버의 내 기록이 '
