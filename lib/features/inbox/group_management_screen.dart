...
class _GroupManagementScreenState extends State<GroupManagementScreen> {
  ...
  Future<void> _openAddMember(CoachGroup g) async {
    ...
    try {
      final ok = await HkSheet.show<bool>(
        ...
        builder: (ctx) => Padding(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
              HkSectionLabel('회원 추가 · ${g.name.toUpperCase()}'),
              const SizedBox(height: HyphenTokens.sp2),
              TextField(
                controller: hashCtrl,
                decoration: const InputDecoration(
      ...
      try {
        await repo.addGroupMember(gymId: gym.id, groupId: g.id, memberHash: h);
        Haptic.medium();
        if (!mounted) return;
        _reload();
      } on AppException catch (e) {
      ...
      children: [
        Row(
          children: [
            ...
            Text('$cap명', style: HyphenTokens.micro),
          ],
        ),
        ...
        if (g.description.isNotEmpty) ...[
          const SizedBox(height: 2),
          Text(g.description, style: HyphenTokens.caption),
        ],
        const SizedBox(height: HyphenTokens.sp2),
        HkButton.secondary('회원 추가', onPressed: () => _openAddMember(g)),
      ],
    );
  }
}
